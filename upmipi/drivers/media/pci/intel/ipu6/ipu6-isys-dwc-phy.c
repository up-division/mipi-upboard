...
/*
 * Runtime DWC D-PHY tuning knobs for high-speed MIPI debug.
 *
 * -1 keeps the original table-based behavior.
 * Set dwc_hsfreq_index_override to force an entry in freqranges[].
 * Set dwc_osc_freq_target_override to force DDL target oscillation
 * frequency written to IFC registers 0xe2/0xe3. The valid range is
 * 0x000 ~ 0xfff because the target is programmed as 12 bits.
 *
 * These values are applied when DPHY is configured. Stop streaming, update
 * the parameter, then restart streaming to make the new values take effect.
 */

static int dwc_mbps_override = -1;
module_param_named(dwc_mbps_override, dwc_mbps_override, int, 0644);
MODULE_PARM_DESC(dwc_mbps_override,
		 "Override DWC D-PHY mbps from link_freq, -1 disables override");

static int dwc_hsfreq_index_override = -1;
module_param_named(dwc_hsfreq_index_override, dwc_hsfreq_index_override,
		   int, 0644);
MODULE_PARM_DESC(dwc_hsfreq_index_override,
		 "Override DWC D-PHY hsfreq table index, -1 disables override");

static int dwc_osc_freq_target_override = -1;
module_param_named(dwc_osc_freq_target_override, dwc_osc_freq_target_override,
		   int, 0644);
MODULE_PARM_DESC(dwc_osc_freq_target_override,
		 "Override DWC D-PHY osc_freq_target, -1 disables override");

static int dwc_deskew_polarity_rw_override = -1;
module_param_named(dwc_deskew_polarity_rw_override,
		   dwc_deskew_polarity_rw_override, int, 0644);
MODULE_PARM_DESC(dwc_deskew_polarity_rw_override,
		 "Override DWC D-PHY deskew_polarity_rw: -1 original, 0 off, 1 on");

static const struct dwc_dphy_freq_range freqranges[DPHY_FREQ_RANGE_NUM] = {
	{0x00,	80,		97,		80,		335}, //0
	{0x10,	80,		107,	90,		335}, //1
	{0x20,	84,		118,	100,	335}, //2
	{0x30,	93,		128,	110,	335}, //3
	{0x01,	103,	139,	120,	335}, //4
	{0x11,	112,	149,	130,	335}, //5
	{0x21,	122,	160,	140,	335}, //6
	{0x31,	131,	170,	150,	335}, //7
	{0x02,	141,	181,	160,	335}, //8
	{0x12,	150,	191,	170,	335}, //9
	{0x22,	160,	202,	180,	335}, //10
	{0x32,	169,	212,	190,	335}, //11
	{0x03,	183,	228,	205,	335}, //12
	{0x13,	198,	244,	220,	335}, //13
	{0x23,	212,	259,	235,	335}, //14
	{0x33,	226,	275,	250,	335}, //15
	{0x04,	250,	301,	275,	335}, //16
	{0x14,	274,	328,	300,	335}, //17
	{0x25,	297,	354,	325,	335}, //18
	{0x35,	321,	380,	350,	335}, //19
	{0x05,	369,	433,	400,	335}, //20
	{0x16,	416,	485,	450,	335}, //21
	{0x26,	464,	538,	500,	335}, //22
	{0x37,	511,	590,	550,	335}, //23
	{0x07,	559,	643,	600,	335}, //24
	{0x18,	606,	695,	650,	335}, //25
	{0x28,	654,	748,	700,	335}, //26
	{0x39,	701,	800,	750,	335}, //27
	{0x09,	749,	853,	800,	335}, //28
	{0x19,	796,	905,	850,	335}, //29
	{0x29,	844,	958,	900,	335}, //30
	{0x3a,	891,	1010,	950,	335}, //31
	{0x0a,	939,	1063,	1000,	335}, //32
	{0x1a,	986,	1115,	1050,	335}, //33
	{0x2a,	1034,	1168,	1100,	335}, //34
	{0x3b,	1081,	1220,	1150,	335}, //35
	{0x0b,	1129,	1273,	1200,	335}, //36
	{0x1b,	1176,	1325,	1250,	335}, //37
	{0x2b,	1224,	1378,	1300,	335}, //38
	{0x3c,	1271,	1430,	1350,	335}, //39
	{0x0c,	1319,	1483,	1400,	335}, //40
	{0x1c,	1366,	1535,	1450,	335}, //41
	{0x2c,	1414,	1588,	1500,	335}, //42
	{0x3d,	1461,	1640,	1550,	208}, //43
	{0x0d,	1509,	1693,	1600,	214}, //44
	{0x1d,	1556,	1745,	1650,	221}, //45
	{0x2e,	1604,	1798,	1700,	228}, //46
	{0x3e,	1651,	1850,	1750,	234}, //47
	{0x0e,	1699,	1903,	1800,	241}, //48
	{0x1e,	1746,	1955,	1850,	248}, //49
	{0x2f,	1794,	2008,	1900,	255}, //50
	{0x3f,	1841,	2060,	1950,	261}, //51
	{0x0f,	1889,	2113,	2000,	268}, //52
	{0x40,	1936,	2165,	2050,	275}, //53
	{0x41,	1984,	2218,	2100,	281}, //54
	{0x42,	2031,	2270,	2150,	288}, //55
	{0x43,	2079,	2323,	2200,	294}, //56
	{0x44,	2126,	2375,	2250,	302}, //57
	{0x45,	2174,	2428,	2300,	308}, //58
	{0x46,	2221,	2480,	2350,	315}, //59
	{0x47,	2269,	2500,	2400,	321}, //60
	{0x48,	2316,	2500,	2450,	328}, //61
...
// 	{0x1c,	1366,	1535,	1450,	335}, //41
// 	{0x2c,	1414,	1588,	1500,	335}, //42
// 	{0x3d,	1461,	1640,	1550,	335}, //43
// 	{0x0d,	1509,	1693,	1600,	335}, //44
// 	{0x1d,	1556,	1745,	1650,	335}, //45
// 	{0x2e,	1604,	1798,	1700,	228}, //46
// 	{0x3e,	1651,	1850,	1750,	234}, //47
// 	{0x0e,	1699,	1903,	1800,	241}, //48
// 	{0x1e,	1746,	1955,	1850,	248}, //49
...
static u16 get_hsfreq_by_mbps(u32 mbps)
{
	...
	/* First prefer exact default_mbps match. */
	for (i = 0; i < DPHY_FREQ_RANGE_NUM; i++) {
		if (freqranges[i].default_mbps == mbps)
			return i;
	}

	/* Then fallback to range match. */
	for (i = 0; i < DPHY_FREQ_RANGE_NUM; i++) {
				    ...
				    u32 phy_id, u32 mbps)
{
	...
	u32 osc_freq_target;
	u32 index;
	bool apply_deskew_polarity_rw;

	dev_info(dev, "[IPU-DWC] config DPHY phy_id=%u mbps=%u\n",
		 phy_id, mbps);

	index = get_hsfreq_by_mbps(mbps);
	if (index == DPHY_FREQ_RANGE_INVALID_INDEX) {
		dev_err(dev, "[IPU-DWC] link freq not found for mbps=%u\n",
			mbps);
		return -EINVAL;
	}

	if (dwc_hsfreq_index_override >= 0) {
		if (dwc_hsfreq_index_override >= DPHY_FREQ_RANGE_NUM) {
			dev_err(dev,
				"[IPU-DWC] invalid hsfreq_index_override=%d valid=-1..%u\n",
				dwc_hsfreq_index_override,
				DPHY_FREQ_RANGE_NUM - 1);
			return -EINVAL;
		}

		dev_info(dev,
			 "[IPU-DWC] phy_id=%u mbps=%u override hsfreq_index %u -> %d\n",
			 phy_id, mbps, index, dwc_hsfreq_index_override);
		index = dwc_hsfreq_index_override;
	}

	dev_info(dev,
		 "[IPU-DWC] phy_id=%u mbps=%u hsfreq_index=%u hsfreq=0x%x table_osc_freq_target=0x%x\n",
		 phy_id,
		 mbps,
		 index,
		 freqranges[index].hsfreq,
	...
	if (isys->phy_termcal_val) {
		dev_info(dev,
			 "[IPU-DWC] phy_id=%u force termcal phy_termcal_val=0x%x\n",
			 phy_id, isys->phy_termcal_val);
	...
	/*
	 * Configure registers 0xe2, 0xe3 with the appropriate
	 * DDL target oscillation frequency.
	 */
	osc_freq_target = freqranges[index].osc_freq_target;
	if (dwc_osc_freq_target_override >= 0) {
		if (dwc_osc_freq_target_override > 0xfff) {
			dev_err(dev,
				"[IPU-DWC] invalid osc_freq_target_override=0x%x valid=-1..0xfff\n",
				dwc_osc_freq_target_override);
			return -EINVAL;
		}

		dev_info(dev,
			 "[IPU-DWC] phy_id=%u mbps=%u override osc_freq_target 0x%x -> 0x%x\n",
			 phy_id, mbps, osc_freq_target,
			 dwc_osc_freq_target_override);
		osc_freq_target = dwc_osc_freq_target_override;
	}

	dev_info(dev,
		 "[IPU-DWC] phy_id=%u mbps=%u final hsfreq_index=%u hsfreq=0x%x final_osc_freq_target=0x%x\n",
		 phy_id, mbps, index, freqranges[index].hsfreq,
		 osc_freq_target);
	 ...
	 * deskew_polarity_rw is applied only for < 1.5Gbps.
	 * Keep that as default, but allow runtime override for high-speed debug.
	 */
	apply_deskew_polarity_rw = mbps < 1500;
	if (dwc_deskew_polarity_rw_override >= 0) {
		if (dwc_deskew_polarity_rw_override > 1) {
			dev_err(dev,
				"[IPU-DWC] invalid deskew_polarity_rw_override=%d valid=-1/0/1\n",
				dwc_deskew_polarity_rw_override);
			return -EINVAL;
		}

		apply_deskew_polarity_rw = dwc_deskew_polarity_rw_override;
		dev_info(dev,
			 "[IPU-DWC] phy_id=%u mbps=%u override deskew_polarity_rw apply=%s\n",
			 phy_id, mbps,
			 apply_deskew_polarity_rw ? "yes" : "no");
	}

	dev_info(dev,
		 "[IPU-DWC] phy_id=%u mbps=%u deskew_polarity_rw apply=%s\n",
		 phy_id, mbps,
		 apply_deskew_polarity_rw ? "yes" : "no");

	if (apply_deskew_polarity_rw)
	...
	dev_info(dev,
		 "[IPU-DWC] phy_id=%u ref_clk=%u cfg_clk_freqrange=0x%x\n",
		 phy_id,
		 isp->buttress.ref_clk,
		 cfg_clk_freqrange);
	...
	dev_info(dev,
		 "[IPU-DWC] config done phy_id=%u mbps=%u\n",
		 phy_id, mbps);
				...
				bool on)
{
	...
	if (dwc_mbps_override >= 0) {
		dev_info(dev, "[IPU-DWC] override mbps %u -> %d\n",
			mbps, dwc_mbps_override);
		mbps = dwc_mbps_override;
	}
