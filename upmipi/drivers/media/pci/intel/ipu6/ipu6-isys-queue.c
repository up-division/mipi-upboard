...
static int ipu6_isys_link_fmt_validate(struct ipu6_isys_queue *aq)
{
	...
	if (!remote_pad) {
		dev_err(dev, "[IPU-CHK] q_link_fmt_validate: no remote pad\n");
		return -ENOTCONN;
	}
	...
	dev_info(dev,
		 "[IPU-CHK] q_link_fmt_validate: vdev=%s remote=%s pad=%u r_stream=%u\n",
		 av->vdev.name, sd->entity.name, remote_pad->index, r_stream);

	ret = ipu6_isys_get_stream_pad_fmt(sd, remote_pad->index, r_stream, &format);
	if (ret) {
		dev_err(dev,
			"[IPU-CHK] q_link_fmt_validate: failed to get %s pad=%u stream=%u fmt ret=%d\n",
			sd->entity.name, remote_pad->index, r_stream, ret);
		return ret;
	}
	...
	dev_info(dev,
		 "[IPU-CHK] q_link_fmt_validate: src fmt=%ux%u code=0x%x, req fmt=%ux%u code=0x%x\n",
		 format.width, format.height, format.code,
		 ipu6_isys_get_frame_width(av),
		 ipu6_isys_get_frame_height(av),
		 code);

	/* 先不要擋，直接放行，確認 dev33 是不是卡在這裡 */
	dev_info(dev,
		 "[IPU-CHK] q_link_fmt_validate: force pass for vdev=%s\n",
		 av->vdev.name);
...
static int start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	dev_info(dev,
		 "[IPU-CHK] STREAMON enter: vdev=%s width=%u height=%u css_pf=%u count=%u\n",
		 av->vdev.name,
		 ipu6_isys_get_frame_width(av),
		 ipu6_isys_get_frame_height(av),
		 pfmt->css_pixelformat, count);

	ret = ipu6_isys_setup_video(av, &source_entity, &nr_queues);
	dev_info(dev,
		 "[IPU-CHK] after setup_video: vdev=%s ret=%d stream=%p source_entity=%s nr_queues=%d vc=%u source_stream=%u\n",
		 av->vdev.name, ret, av->stream,
		 source_entity ? source_entity->name : "(null)",
		 nr_queues, av->vc, av->source_stream);
	if (ret < 0) {
		dev_err(dev, "[IPU-CHK] failed to setup video: %d\n", ret);
		goto out_return_buffers;
	}

	ret = ipu6_isys_link_fmt_validate(aq);
	dev_info(dev, "[IPU-CHK] after q_link_fmt_validate: vdev=%s ret=%d\n",
		 av->vdev.name, ret);
	if (ret) {
		dev_err(dev, "[IPU-CHK] link format validation failed (%d)\n", ret);
		goto out_pipeline_stop;
	}

	ret = ipu6_isys_fw_open(av->isys);
	dev_info(dev, "[IPU-CHK] after fw_open: vdev=%s ret=%d\n",
		 av->vdev.name, ret);
	if (ret)
		goto out_pipeline_stop;
	...
	dev_info(dev,
		 "[IPU-CHK] stream state before prepare: vdev=%s stream=%p nr_streaming=%u nr_queues=%u queues_empty=%d\n",
		 av->vdev.name, stream, stream->nr_streaming, stream->nr_queues,
		 list_empty(&stream->queues));

	if (!stream->nr_streaming) {
		ret = ipu6_isys_video_prepare_stream(av, source_entity, nr_queues);
		dev_info(dev,
			 "[IPU-CHK] after prepare_stream: vdev=%s ret=%d stream=%p\n",
			 av->vdev.name, ret, stream);
		if (ret)
			goto out_fw_close;
	}

	stream->nr_streaming++;
	dev_info(dev, "[IPU-CHK] queue %u of %u for vdev=%s\n",
		 stream->nr_streaming, stream->nr_queues, av->vdev.name);
	...
	dev_info(dev,
		 "[IPU-CHK] after list_add: vdev=%s nr_streaming=%u nr_queues=%u\n",
		 av->vdev.name, stream->nr_streaming, stream->nr_queues);
	...
	bl = &__bl;
	ret = buffer_list_get(stream, bl);
	dev_info(dev, "[IPU-CHK] after buffer_list_get: vdev=%s ret=%d bl=%p\n",
		 av->vdev.name, ret, bl);
	if (ret < 0) {
		dev_warn(dev, "[IPU-CHK] no buffer available, DRIVER BUG?\n");
		goto out;
	}

	ret = ipu6_isys_stream_start(av, bl, false);
	dev_info(dev, "[IPU-CHK] after ipu6_isys_stream_start: vdev=%s ret=%d\n",
		 av->vdev.name, ret);
	if (ret)
...
out_return_buffers:
	...
	dev_err(dev, "[IPU-CHK] STREAMON exit with error: vdev=%s ret=%d\n",
		av->vdev.name, ret);
	return ret;
}
