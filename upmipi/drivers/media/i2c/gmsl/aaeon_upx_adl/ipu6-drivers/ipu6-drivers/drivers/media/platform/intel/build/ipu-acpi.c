...
#include "ipu-acpi.h"
#include "ipu-get-acpi.h"

// #if IS_ENABLED(CONFIG_VIDEO_UPX_MTL)
#define ISX031_I2C_ADDRESS 0x1a
#define SHW5G_I2C_ADDRESS 0x10
#define ISX031F_NAME "isx031f"
#define SHW5G_NAME "shw5g"
// #endif
...
// #endif
// #if IS_ENABLED(CONFIG_VIDEO_UPX_MTL)
	{ "SGMX3M00", ISX031F_NAME, get_sensor_pdata, NULL, 0, TYPE_SERDES, "max9x",
		ISX031_I2C_ADDRESS, 1600 },
	{ "SGSHW5G", SHW5G_NAME, get_sensor_pdata, NULL, 0, TYPE_SERDES, "max9x",
		SHW5G_I2C_ADDRESS, 1600 },
// #endif
// #endif
};
...
static int __init ipu_acpi_init(void)
{
	...
	printk(KERN_INFO "\n============================================\n");
	printk(KERN_INFO "[ACPI DUMPER] 開始主動掃描 BIOS ACPI 表格...\n");
	
	set_built_in_pdata(NULL);
	
	/* 主動呼叫掃描函數，它會印出一大堆 ACPI 資訊到 dmesg */
	ipu_get_acpi_devices_new(&spdata);
	
	if (spdata) {
		printk(KERN_INFO "[ACPI DUMPER] 掃描成功！成功獲取 platform_data 結構。\n");
	} else {
		printk(KERN_ERR "[ACPI DUMPER] 掃描失敗：回傳的 platform_data 為 NULL。\n");
	}
	printk(KERN_INFO "============================================\n\n");
