...
static const char *const max96717_gpio_chip_names[] = {
	"MFP1",
	"MFP2",
	"MFP3",
	"MFP4",
...
static int max96717_setup_gpio(struct max9x_common *common)
{
	...
	if (gpio_pdata && gpio_pdata->names)
		...
		common->gpio_chip.names = max96717_gpio_chip_names;
...
static int max96717_set_stream_id(struct max9x_common *common)
{
	struct device_node *node = common->dev->of_node;
	struct device *dev = common->dev;
	struct regmap *map = common->map;
	...
	if(common->client->addr == 0x44)
		stream_id = 0x10;
	else if(common->client->addr == 0x45)
		stream_id = 0x11;
	else if(common->client->addr == 0x46)
		stream_id = 0x12;
	else if(common->client->addr == 0x47)
		stream_id = 0x13;
	else
		dev_err(dev, "Unexpected I2C address, cannot set stream id");

	dev_info(dev, "---------------------------------Stream ID: 0x%02x", stream_id);

	ret = regmap_write(map, 0x5B, stream_id);
	if (ret) {
		dev_err(dev, "Failed to write des frame sync id with err: %d",
			ret);
		return ret;
...
static int max96717_enable(struct max9x_common *common)
{
	...
	dev_dbg(dev, "set stream id");
	ret = max96717_set_stream_id(common);
	if (ret)
		return ret;
	...
	for (alias = 0; alias < MAX96717_NUM_ALIASES; alias++) {
		...
		if (src == virt_addr || src == 0) {
			dev_dbg(dev, "SRC %02x = %02x, DST %02x = %02x",
				MAX96717_I2C_SRC(i2c_id, alias), virt_addr,
				MAX96717_I2C_DST(i2c_id, alias), phys_addr);
			TRY(ret, regmap_write_retry(map, MAX96717_I2C_DST(i2c_id, alias),
					      MAX9X_FIELD_PREP(MAX96717_I2C_DST_FIELD, phys_addr))
			);

			TRY(ret, regmap_write_retry(map, MAX96717_I2C_SRC(i2c_id, alias),
