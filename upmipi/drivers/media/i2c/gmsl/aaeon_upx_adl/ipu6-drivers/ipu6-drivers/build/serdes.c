...
static const struct of_device_id max9x_of_match[] = {
	...
	{ .compatible = "max9x,max96724f", .data = &max9x_chips[MAX96724F] },
	{ .compatible = "max9x,max9295", .data = &max9x_chips[MAX9295] },
	{ .compatible = "max9x,max96717", .data = &max9x_chips[MAX96717] },
	{ .compatible = "max9x,max96717f", .data = &max9x_chips[MAX96717F] },
	{}
};
...
static void *parse_serdes_pdata(struct device *dev)
{
	...
	for (unsigned int serial_link_id = 0; serial_link_id < des_pdata->num_serial_links; serial_link_id++) {
		...
		printk("------------------------------------ ser_alias: %02x\n", ser_alias);
		printk("------------------------------------ sensor_alias: %02x\n", sensor_alias);
		printk("------------------------------------ ser_phys_addr: %02x\n", ser_phys_addr);
		printk("------------------------------------ sensor_phys_addr: %02x\n", sensor_phys_addr);
		printk("------------------------------------ lanes: %d\n", lanes);
		printk("------------------------------------ dt: %02x\n", dt);
		...
		if (strcmp(sensor_name, "isx031f") == 0) {
			serial_link->rx_freq_mhz = 3000;
		}
	}
		...
		if (common->type == MAX9X_DESERIALIZER) {
			...
			for (unsigned int i = 0; i < pdata->num_subdevs; i++) {
				...
				if (subdev_pdata->serial_link_id == link_id) {
					...
					// snprintf(dev_id, sizeof(dev_id), "%d-%04x", common->muxc->adapter[link_id]->nr, subdev_pdata->board_info.addr << 1);
