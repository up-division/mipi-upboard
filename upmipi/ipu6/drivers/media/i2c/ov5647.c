...
/* MIPI CSI-2 設定 */
#define OV5647_NUM_DATA_LANES      2
#define OV5647_BITS_PER_PIXEL      10
#define OV5647_LINK_FREQ_2592X1944 218750000ULL  /* 87.5MHz */
#define OV5647_LINK_FREQ_1080P     204166750ULL  /* 81.6667MHz */
#define OV5647_LINK_FREQ_BINNING   204166750ULL  /* 跟 1080p 同一組 PLL */
#define OV5647_LINK_FREQ_VGA       137500000ULL  /* 55MHz */



enum ov5647_link_freq_index {
	OV5647_LINK_FREQ_IDX_2592X1944 = 0,
	OV5647_LINK_FREQ_IDX_1080P,
	OV5647_LINK_FREQ_IDX_BINNING,
	OV5647_LINK_FREQ_IDX_VGA,
	OV5647_LINK_FREQ_IDX_MAX,
};

static const s64 ov5647_link_freqs[] = {
	[OV5647_LINK_FREQ_IDX_2592X1944] = OV5647_LINK_FREQ_2592X1944,
	[OV5647_LINK_FREQ_IDX_1080P]     = OV5647_LINK_FREQ_1080P,
	[OV5647_LINK_FREQ_IDX_BINNING]   = OV5647_LINK_FREQ_BINNING,
	[OV5647_LINK_FREQ_IDX_VGA]       = OV5647_LINK_FREQ_VGA,
};
...
static const struct regval_list ov5647_mipi_timing_common[] = {
	{0x4800, 0x24}, /* MIPI control: HS, lane, etc. */
	{0x4805, 0x10}, /* HS prepare / trail */
	{0x4814, 0x2a}, /* HS zero / exit */
	{0x4827, 0x16}, /* LP-11 / LPX period */
	/* 0x4837 (global timing) 依 mode 不同設定 */
};
...
static const struct ov5647_mode ov5647_modes[] = {
	/* 0: 2592x1944 full resolution full FOV 10-bit mode. */
	{
		...
		.hts		= 2844,
		.vts		= 0x7b0,
		.link_freq_index = OV5647_LINK_FREQ_IDX_2592X1944,
		.reg_list	= ov5647_2592x1944_10bpp,
		.num_regs	= ARRAY_SIZE(ov5647_2592x1944_10bpp)
	},
	/* 1: 1080p30 10-bit mode. */
	{
		.format = {
			.code		= MEDIA_BUS_FMT_SBGGR10_1X10,
			.colorspace	= V4L2_COLORSPACE_SRGB,
		...
		.hts		= 2416,
		.vts		= 0x450,
		.link_freq_index = OV5647_LINK_FREQ_IDX_1080P,
		.reg_list	= ov5647_1080p30_10bpp,
		.num_regs	= ARRAY_SIZE(ov5647_1080p30_10bpp)
	},
	/* 2: 2x2 binned full FOV 10-bit mode. */
	{
		.format = {
			.code		= MEDIA_BUS_FMT_SBGGR10_1X10,
			.colorspace	= V4L2_COLORSPACE_SRGB,
		...
		.hts		= 1896,
		.vts		= 0x59b,
		.link_freq_index = OV5647_LINK_FREQ_IDX_BINNING,
	...
	/* 3: 10-bit VGA ... */
	{
		...
		.hts		= 1852,
		.vts		= 0x1f8,
		.link_freq_index = OV5647_LINK_FREQ_IDX_VGA,
...
/* 從 link-freq 推 pixel_rate，公式類似 ov2740 */
static u64 ov5647_calc_pixel_rate(unsigned int freq_index)
{
	u64 rate;

	if (freq_index >= ARRAY_SIZE(ov5647_link_freqs))
		freq_index = 0;

	/* IPU6 ：pixel_rate = link_freq * 2(DDR) * lanes / bpp */
	rate = ov5647_link_freqs[freq_index];
	rate *= 2 * OV5647_NUM_DATA_LANES;
	do_div(rate, OV5647_BITS_PER_PIXEL);
...
static void ov5647_update_timing_ctrls(struct ov5647 *sensor)
{
	u64 prate;
	int hblank;
	int vblank_def;
	unsigned int lf_idx = sensor->mode->link_freq_index;

	/* link_freq control -> index */
	if (sensor->link_freq)
		__v4l2_ctrl_s_ctrl(sensor->link_freq, lf_idx);

	/* pixel_rate 從 link_freq 算 */
	prate = ov5647_calc_pixel_rate(lf_idx);
	if (sensor->pixel_rate)
		__v4l2_ctrl_s_ctrl_int64(sensor->pixel_rate, prate);
	...
	/* vblank = vts - height */
	vblank_def = sensor->mode->vts - sensor->mode->format.height;
	if (sensor->vblank)
		__v4l2_ctrl_s_ctrl(sensor->vblank, vblank_def);
}

static void ov5647_apply_mode(struct ov5647 *sensor,
			      const struct ov5647_mode *mode)
{
	...
	/* 先更新 H/VBLANK & pixel_rate/link_freq */
	ov5647_update_timing_ctrls(sensor);
...
static int ov5647_get_mbus_config(struct v4l2_subdev *sd,
				  unsigned int pad,
				  struct v4l2_mbus_config *cfg)
{
	...
	/* 新版 API：從 cfg->bus.mipi_csi2 設定 lane 資訊 */
	cfg->bus.mipi_csi2.num_data_lanes = 2;
	cfg->bus.mipi_csi2.data_lanes[0] = 0;
	cfg->bus.mipi_csi2.data_lanes[1] = 1;
...
static int ov5647_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_LINK_FREQ:
		/* 給 IPU6 / 上層看的 meta control，不需要寫任何暫存器 */
		break;

	case V4L2_CID_PIXEL_RATE:
	case V4L2_CID_HBLANK:
		/* read-only，僅由 driver 更新，不寫 HW */
		break;
...
//			  V4L2_CID_ANALOGUE_GAIN, 16, 1023, 1, 32);
	...
	sensor->link_freq = v4l2_ctrl_new_int_menu(
		...
		lf_idx,                /* default: 目前 mode 的 index */
		ov5647_link_freqs);

	if (sensor->link_freq)
		sensor->link_freq->flags |= V4L2_CTRL_FLAG_READ_ONLY;

	/* ====== PIXEL_RATE：由 link_freq 推算 ====== */
	prate = ov5647_calc_pixel_rate(lf_idx);

	sensor->pixel_rate = v4l2_ctrl_new_std(&sensor->ctrls,
					       &ov5647_ctrl_ops,
					       V4L2_CID_PIXEL_RATE,
					       prate, prate, 1, prate);
...
#ifdef CONFIG_ACPI
static const struct acpi_device_id ov5647_acpi_ids[] = {
	{ "OV5647", 0 },
	{}
};
MODULE_DEVICE_TABLE(acpi, ov5647_acpi_ids);
#else
...
static struct i2c_driver ov5647_i2c_driver = {
	.driver = {
		.name = "ov5647",
#ifdef CONFIG_ACPI
		.acpi_match_table = ACPI_PTR(ov5647_acpi_ids),
#else
